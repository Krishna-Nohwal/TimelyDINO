"""
train_stage1_4layers.py — Stage 1 training script (4-layer variant).
...
class SpatialHead(nn.Module):
    """
    Spatial head using CLS, flattened register tokens, and attention-pooled
    patch tokens.

    Fused input: CLS(C) + REG(4C) + PATCH(C) = 6C.
    The final classifier is a two-linear-layer MLP with ReLU in between.
    """
    ...
    def forward(self, cls_tok, reg_tok, patch_tok):
        ...
        f_reg = reg_tok.reshape(B, self.num_reg * reg_tok.size(-1))
        f_patch = self.patch_pool(patch_tok)

        fused = torch.cat(
            [f_cls.float(), f_reg.float(), f_patch.float()],
            dim=1,
        )
        features = self.mlp[1](self.mlp[0](fused))
        logits = self.mlp[2](features)
...
class ViT(nn.Module):
    """
    DINOv3 ViT-Large/16 frame model with 4 tapped layers [20, 21, 22, 23].

    Register tokens are flattened to (B, 4C) instead of mean-pooled. Patch
    tokens use attention pooling, matching the main frame_model behavior.
    """
...
def check_layerscale(vit_backbone):
    """
    DINOv2/v3 ViT variants in timm normally ship with LayerScale (ls1/ls2)
    built into every block already. We don't add it manually — we just
    verify it's present so we're not silently missing it.
    """
    try:
        ...
        print(f"  Could not verify LayerScale presence: {e}")
        return None
...
parser.add_argument('--save_root',     default='checkpoints_reg_flat_mlp', type=str)
...
if __name__ == "__main__":
    ...
    # Verify LayerScale is present in the DINOv3 backbone (should be native;
    # we don't add it manually, just confirm it's there).
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imgs, labels) in enumerate(
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16):
                ...
                # Primary supervision on the deepest tapped layer (index 3,
                # i.e. transformer block 23). Aux supervision averages the
                # other 3 tapped layers (blocks 20, 21, 22).
