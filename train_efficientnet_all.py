"""
Train a pretrained EfficientNet frame classifier on FF++ plus extra datasets.

This uses the same dataset handling as train_xception_ffpp.py:
    FF++, CDFv2, CDF++, UADFV, DFo, DFD, DFDC, and WDF.

Example:
python train_efficientnet_all.py \
    ...
    --save_dir checkpoints_efficientnet_all \
    --frames_per_video 8 \
    --epochs 10 \
    --batch_size 32
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train pretrained EfficientNet on FF++ plus optional extra datasets.")
    ...
    p.add_argument("--save_dir", default="checkpoints_efficientnet_all")
    p.add_argument("--model_name", default="tf_efficientnet_b4_ns",
                   help="timm EfficientNet name, e.g. efficientnet_b0, tf_efficientnet_b4_ns.")
...
def main():
    ...
    print("=" * 88)
    print("Train pretrained EfficientNet on FF++ + extra datasets")
    print("=" * 88)
    ...
    model = build_efficientnet(args.model_name).to(device)
