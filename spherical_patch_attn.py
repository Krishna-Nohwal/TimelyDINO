"""
Spherical patch-attention Stage 1 training script.

This is a controlled variant of patch_attn.py. It uses only final-layer patch
tokens, attention-pools them into one image feature, then replaces the learned
linear classifier with fixed simplex prototypes on the unit hypersphere:

  patch_feat = AttentionPool(final_layer_patch_tokens)
  sphere_z   = projector(patch_feat), L2-normalized
  logits     = cosine logits to fixed real/fake simplex prototypes
  loss       = CrossEntropy(logits, labels) + lambda * SupCon(sphere_z, labels)
...
parser = argparse.ArgumentParser(
    description="Stage 1 spherical patch-attention training: final-layer patch tokens + fixed simplex prototypes"
)
parser.add_argument("--epochs",        default=50,   type=int)
parser.add_argument("--batch_size",    default=128,  type=int)
parser.add_argument("--num_workers",   default=36,   type=int)
parser.add_argument("--save_root",     default="checkpoints_spherical_patch_attn", type=str)
...
class FixedSphericalHead(nn.Module):
    """Project patch-attention features onto a hypersphere and score fixed anchors."""
...
class SphericalPatchAttnViT(nn.Module):
    """DINOv3 ViT-L/16 using attention-pooled patch tokens and fixed spherical prototypes."""
    ...
    def forward(self, x):
        ...
        logits, sphere_features = self.spherical_head(patch_feat)
...
def spherical_patch_attn_loss(logits, sphere_features, labels, supcon_weight):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imgs, labels) in enumerate(
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16):
                ...
                loss = spherical_patch_attn_loss(logits, sphere_features, labels, args.supcon_weight)
