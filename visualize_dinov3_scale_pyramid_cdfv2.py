"""
Visualize DINOv3 embeddings across a scale pyramid on CDFv2 images.

For each sampled CDFv2 frame image:
...
Example
-------
python visualize_dinov3_scale_pyramid_cdfv2.py \
  ...
  --num_images 50 \
  --output_dir dinov3_scale_pyramid_cdfv2 \
  --input_size 256 \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="DINOv3 scale-pyramid similarity/UMAP visualization on CDFv2."
    )
    parser.add_argument(
        "--fake_root",
    ...
    parser.add_argument("--output_dir", default="dinov3_scale_pyramid_cdfv2")
...
def load_cdfv2_items(fake_root: str, real_root: str) -> list[dict]:
    items = []
    for root_str, label in [(fake_root, 1), (real_root, 0)]:
        ...
        for sample_dir in sorted(root.iterdir()):
            ...
            if sample_dir.is_dir() and img_path.is_file():
                ...
                items.append({
                    ...
                    "video_id": video_id_from_sample_dir(sample_name),
                    "label": label,
                })
                found += 1
            else:
        ...
        print(f"Loaded CDFv2 {label_name}: found={found} skipped={skipped} root={root}")
    return items
...
def main():
    ...
    print("=" * 88)
    print("DINOv3 scale-pyramid visualization on CDFv2")
    print("=" * 88)
    ...
    items = load_cdfv2_items(args.fake_root, args.real_root)
