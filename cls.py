"""
CLS-only Stage 1 training script.

This trains a DINOv3 ViT-L/16 model using only the final-layer CLS token:

  logits = classifier(cls_last)
  loss   = CrossEntropy(logits, labels) + lambda * SupCon(cls_last, labels)

There are no patch/reg-token heads, no auxiliary tapped layers, and no
...
parser = argparse.ArgumentParser(
    description="Stage 1 CLS-only training: final-layer CLS token + CE/SupCon"
)
parser.add_argument("--epochs",        default=50,   type=int)
parser.add_argument("--batch_size",    default=128,  type=int)
parser.add_argument("--num_workers",   default=36,   type=int)
parser.add_argument("--save_root",     default="checkpoints_cls", type=str)
...
class CLSViT(nn.Module):
    """DINOv3 ViT-L/16 using only final-layer CLS for classification."""
...
def cls_loss(logits, cls_features, labels, supcon_weight):
    return ce_loss(logits, labels) + supcon_weight * supcon_loss(cls_features, labels)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imgs, labels) in enumerate(
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16):
                logits, cls_features = model(imgs)
                loss = cls_loss(logits, cls_features, labels, args.supcon_weight)
