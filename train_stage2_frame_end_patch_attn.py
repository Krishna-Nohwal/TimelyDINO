...
parser.add_argument("--save_root",        default="checkpoints_s2_frame_end", type=str)
parser.add_argument("--load_from",        default="main_checkpoint/best.pth",   type=str,
...
class VideoViT(nn.Module):
    ...
    def frame_model_patch_attn(self, frames: Tensor):
        """
        Run the frozen Stage-1 model and return per-layer patch* vectors.

        patch* is the attention-pooled patch feature produced by each
        SpatialHead.patch_pool before [CLS|REG|patch*] fusion. These are
        1024-d vectors, so they can replace f_cls in the four temporal streams
        and in the real-video memory bank without changing Stage-2 dimensions.
        """
        _, intermediates = self.frame_model.vit.forward_intermediates(
            frames,
            indices=self.frame_model.LAYERS,
        ...
        patch_attn_list: list = []

        for i, (spatial_map, prefix_tokens) in enumerate(intermediates):
            Bf, Cf, Hf, Wf = spatial_map.shape
            patch_tok = spatial_map.permute(0, 2, 3, 1).contiguous().reshape(Bf, Hf * Wf, Cf)
            cls_tok = prefix_tokens[:, :1, :]
            reg_tok = prefix_tokens[:, 1:1 + self.frame_model.NUM_REG, :]

            result = self.frame_model.spatial_heads[i](cls_tok, reg_tok, patch_tok)
            frame_logits_list.append(result["logits"])
            frame_feats_list.append(result["features"])
            patch_attn_list.append(result["f_patch"])

        return frame_logits_list, frame_feats_list, patch_attn_list
    ...
    def forward(
        ...
        frame_logits_list, frame_feats_list, patch_attn_list = self.frame_model_patch_attn(frames)
        ...
        patch_attn_sequences = [
            patch_attn_tokens.reshape(B, T, self.EMBED_DIM)
            for patch_attn_tokens in patch_attn_list
        ]
        ...
        if self.use_memory_bank:
            ...
            memory_refs = self.memory_bank.query(patch_attn_sequences, key_padding_mask)
        else:
            memory_refs = None

        video_feats_list = []
        no_memory_feats_list = [] if return_memory_ablation and memory_refs is not None else None
        for h, (temporal_tfm, frame_patch_attn) in enumerate(zip(self.temporal_transformers, patch_attn_sequences)):
            no_memory_frame_patch_attn = frame_patch_attn
            if memory_refs is not None:
                gate = torch.sigmoid(self.memory_gate[h]).to(dtype=frame_patch_attn.dtype)
                memory_ref = memory_refs[h].unsqueeze(1)
                frame_patch_attn = (1 - gate) * frame_patch_attn + gate * memory_ref
            if no_memory_feats_list is not None:
                if self.training:
                    with torch.no_grad():
                        no_memory_aug = temporal_augment(no_memory_frame_patch_attn, key_padding_mask)
                        no_memory_feats_list.append(
                            temporal_tfm(no_memory_aug, key_padding_mask)
                        )
                else:
                    no_memory_feats_list.append(
                        temporal_tfm(no_memory_frame_patch_attn, key_padding_mask)
                    )
            if self.training:
                frame_patch_attn = temporal_augment(frame_patch_attn, key_padding_mask)
            video_feats_list.append(temporal_tfm(frame_patch_attn, key_padding_mask))
...
def build_memory_bank(
    ...
    """
    Build a RealVideoMemoryBank from frozen patch* prototypes of all real
    training videos.

    Only real videos (label == 0) are stored. Built once before training
    because the frozen frame backbone produces identical patch* embeddings every
    ...
    with torch.inference_mode(), \
        ...
        for frames, labels, lengths in tqdm(real_loader, desc="  Building bank", leave=False):
            ...
            _, _, patch_attn_list = raw_model.frame_model_patch_attn(flat_frames)
            ...
            for patch_attn_tokens in patch_attn_list:
                ...
                frame_patch_attn = patch_attn_tokens.reshape(B, T, raw_model.EMBED_DIM)
                patch_attn_sequences.append(frame_patch_attn)

            bank.add(patch_attn_sequences, key_padding_mask)
    ...
    print(f"  Memory bank ready: {len(bank)} frozen real-video patch* prototypes, k={k}")
    return bank
...
if __name__ == "__main__":
    ...
    print(f"  Backbone: frame_model.ViT  |  Temporal streams: {VideoViT.NUM_TEMPORAL_HEADS} x patch*")
    print(f"{SEP}\n")
