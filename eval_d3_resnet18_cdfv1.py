...
Example
-------
python eval_d3_resnet18_cdfv1.py \
...
Notes
-----
D3's raw second-order score is used as a realness score in the original repo
(`average_precision_score(1 - label, score)`). This script reports the usual
fake-positive metrics by using `-score` as the fake score.
"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="CDFv1 preprocessed root. Frames are read from <cdf_root>/<sample_dir>/image.png.",
    )
    parser.add_argument(
        "--cdf_csv",
    ...
    parser.add_argument("--sampling", default="uniform", choices=["uniform", "first"])
    ...
    parser.add_argument(
        "--no_center_crop",
        action="store_true",
        help="Disable D3-style center crop by 10 percent of the long axis.",
    )
    parser.add_argument(
        "--rgb",
        action="store_true",
        help="Use RGB channel order. By default, this script mimics D3's cv2/BGR behavior.",
    )
    parser.add_argument(
        "--save_results",
...
def sample_frame_indices(n_available: int, n_target: int, sampling: str) -> np.ndarray:
    if n_available <= 0:
...
def load_d3_frame(path: str, center_crop: bool, use_rgb: bool) -> torch.Tensor:
...
class CDFv1D3Dataset(Dataset):
    def __init__(
        ...
        for _, row in df.iterrows():
            ...
            if img_path.is_file():
                ...
                vid2labels[vid].append(int(row["label"]))
            else:
                skipped += 1
    ...
    def __getitem__(self, idx):
        ...
        indices = sample_frame_indices(len(paths), self.num_frames, self.sampling)
        frames = []
        for frame_idx in indices:
            path = paths[int(frame_idx)]
            try:
                frames.append(load_d3_frame(path, self.center_crop, self.use_rgb))
            except Exception:
                frames.append(torch.zeros(3, IMG_SIZE, IMG_SIZE))
        return torch.stack(frames, dim=0), label, vid, len(paths)


def build_fallback_d3_model(loss_type: str):
...
def load_d3_model(args, device: torch.device):
    ...
    import_error = None
    for import_stmt in ("D3_model", "models"):
        try:
            if import_stmt == "D3_model":
    ...
    return build_fallback_d3_model(args.loss).to(device).eval()
...
def main():
    ...
    if dataset.mixed:
        print(f"  [WARNING] Videos with mixed labels after grouping: {dataset.mixed}")
    ...
    with torch.inference_mode(), autocast_ctx:
        for frames, batch_labels, batch_vids, batch_frame_counts in tqdm(
            loader, desc="D3 CDFv1", unit="batch"
        ):
    ...
    for label_value, name in [(0, "real"), (1, "fake")]:
        ...
        print(
            f"    {name:4s} raw score mean={scores.mean():.6f} "
            f"std={scores.std():.6f} min={scores.min():.6f} max={scores.max():.6f}"
        )
