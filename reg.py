"""
Register-token-only Stage 1 training script.

This trains a DINOv3 ViT-L/16 model using only final-layer register tokens:

  reg_feat = mean_pool(final_layer_reg_tokens)
  logits   = classifier(reg_feat)
  loss     = CrossEntropy(logits, labels) + lambda * SupCon(reg_feat, labels)

There are no CLS-token predictions, no patch-token features, no SpatialHeads,
no auxiliary tapped layers, and no MultiSimilarity loss.
"""
...
parser = argparse.ArgumentParser(
    description="Stage 1 reg-token-only training: final-layer register tokens + CE/SupCon"
)
parser.add_argument("--epochs",        default=50,   type=int)
parser.add_argument("--batch_size",    default=128,  type=int)
parser.add_argument("--num_workers",   default=36,   type=int)
parser.add_argument("--save_root",     default="checkpoints_reg", type=str)
...
class RegViT(nn.Module):
    """DINOv3 ViT-L/16 using only final-layer register tokens for classification."""
    ...
    def forward(self, x):
        ...
        reg_tokens = prefix_tokens[:, 1:1 + self.NUM_REG, :]
        reg_feat = reg_tokens.mean(dim=1)
        logits = self.classifier(reg_feat.float())
        return logits, reg_feat
...
def reg_loss(logits, reg_features, labels, supcon_weight):
    return ce_loss(logits, labels) + supcon_weight * supcon_loss(reg_features, labels)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imgs, labels) in enumerate(
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16):
                logits, reg_features = model(imgs)
                loss = reg_loss(logits, reg_features, labels, args.supcon_weight)
