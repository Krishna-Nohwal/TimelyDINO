"""
Patch-only Stage 1 training script.

This trains a DINOv3 ViT-L/16 model using only final-layer patch tokens:

  patch_feat = mean_pool(final_layer_patch_tokens)
...
parser = argparse.ArgumentParser(
    description="Stage 1 patch-only training: final-layer patch tokens + CE/SupCon"
)
parser.add_argument("--epochs",        default=50,   type=int)
parser.add_argument("--batch_size",    default=128,  type=int)
parser.add_argument("--num_workers",   default=36,   type=int)
parser.add_argument("--save_root",     default="checkpoints_patch", type=str)
...
class PatchViT(nn.Module):
    """DINOv3 ViT-L/16 using only final-layer patch tokens for classification."""
    ...
    def forward(self, x):
        ...
        patch_feat = spatial_map.flatten(2).mean(dim=2)
...
def patch_loss(logits, patch_features, labels, supcon_weight):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imgs, labels) in enumerate(
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16):
                ...
                loss = patch_loss(logits, patch_features, labels, args.supcon_weight)
