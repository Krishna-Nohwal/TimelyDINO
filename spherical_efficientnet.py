"""
Spherical EfficientNet Stage 1 training script.

This is an EfficientNet equivalent of spherical_cls.py. It keeps the same
FF++ real/fake manifest split and CDFv1 evaluation, but replaces the DINOv3
CLS token with a pooled EfficientNet feature vector:

  image -> EfficientNet features -> projector -> L2 normalize
        -> cosine logits to fixed simplex prototypes

  loss  = CrossEntropy(logits, labels) + lambda * SupCon(sphere_features, labels)

For the binary real/fake case, the fixed simplex prototypes are antipodal
anchors on the unit hypersphere.
"""
...
parser = argparse.ArgumentParser(
    description="Stage 1 spherical EfficientNet training: pooled features + fixed simplex prototypes"
)
parser.add_argument("--epochs",        default=50,   type=int)
parser.add_argument("--batch_size",    default=64,   type=int)
parser.add_argument("--num_workers",   default=8,    type=int)
parser.add_argument("--save_root",     default="checkpoints_spherical_efficientnet", type=str)
...
class FixedSphericalHead(nn.Module):
    """Project backbone features onto a hypersphere and score fixed anchors."""
...
class SphericalEfficientNet(nn.Module):
    """timm EfficientNet with fixed spherical prototype classification."""
    ...
    def forward(self, x):
        ...
        logits, sphere_features = self.spherical_head(features)
...
def spherical_efficientnet_loss(logits, sphere_features, labels, supcon_weight):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imgs, labels) in enumerate(
            ...
            with autocast_context():
                ...
                loss = spherical_efficientnet_loss(logits, sphere_features, labels, args.supcon_weight)
