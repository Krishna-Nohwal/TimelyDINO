...
* Memory bank is built with num_heads=4 (VideoViT.NUM_TEMPORAL_HEADS) from
  frozen real-video CLS prototypes, then appended to the final classifier
  rather than gated into CLS sequences.
...
parser = argparse.ArgumentParser(
    description="Stage 2: train temporal transformers with frame logits and kNN memory appended at the final classifier"
)
...
parser.add_argument("--save_root",        default="checkpoints_s2_memory_end", type=str)
...
parser.add_argument("--use_memory_bank",  action="store_true",
                    help="Append frozen real-video kNN memory to the final video classifier.")
...
# ---------------------------------------------------------------------------
# Video model with frame-logit and memory shortcuts at the end
# ---------------------------------------------------------------------------

class VideoViT(nn.Module):
    """
    Stage 2 video model with the frame-logit shortcut and kNN memory appended
    at the final video classifier.
    ...
    The final classifier receives:
      ...
      - memory_vec: 4 retrieved memory vectors x 1024 = 4096 dims

    There is no memory gate and memory is not mixed into frame CLS tokens before
    the temporal transformers. During eval we report four ablations by zeroing
    the final frame and/or memory feature blocks before the same classifier.
    """
    ...
    def __init__(
        ...
        self.fusion_classifier = nn.Linear(
            self.NUM_TEMPORAL_HEADS * self.EMBED_DIM * 2 + 2, 2
        )
    ...
    def forward(self, video: Tensor, lengths: Optional[Tensor] = None):
        ...
        if memory_refs is None:
            memory_vec = torch.zeros_like(temporal_vec)
        else:
            memory_vec = torch.cat(
                [ref.to(dtype=temporal_vec.dtype) for ref in memory_refs],
                dim=1,
            )

        zero_frame = torch.zeros_like(frame_mean_logits)
        zero_memory = torch.zeros_like(memory_vec)

        logits_with_all = self.fusion_classifier(
            torch.cat([temporal_vec, frame_mean_logits, memory_vec], dim=1)
        )
        logits_without_frames = self.fusion_classifier(
            torch.cat([temporal_vec, zero_frame, memory_vec], dim=1)
        )
        logits_without_memory = self.fusion_classifier(
            torch.cat([temporal_vec, frame_mean_logits, zero_memory], dim=1)
        )
        logits_without_frames_memory = self.fusion_classifier(
            torch.cat([temporal_vec, zero_frame, zero_memory], dim=1)
        )

        return (
            logits_with_all,
            logits_without_frames,
            logits_without_memory,
            logits_without_frames_memory,
            frame_logits_list,
            frame_feats_list,
            video_feats_list,
        )
...
def _video_probs_from_forward(
    logits_with_all: torch.Tensor,
    logits_without_frames: torch.Tensor,
    logits_without_memory: torch.Tensor,
    logits_without_frames_memory: torch.Tensor,
    frame_logits_list: list,
    labels: torch.Tensor,
    ...
    # Final video classifier ablations.
    probs_with_all = torch.softmax(logits_with_all.float(), dim=1)[:, 1]
    probs_without_frames = torch.softmax(logits_without_frames.float(), dim=1)[:, 1]
    probs_without_memory = torch.softmax(logits_without_memory.float(), dim=1)[:, 1]
    probs_without_frames_memory = torch.softmax(
        logits_without_frames_memory.float(), dim=1
    )[:, 1]
    ...
    return (
        ...
        video_mean_probs,
        probs_with_all,
        probs_without_frames,
        probs_without_memory,
        probs_without_frames_memory,
    )
...
def run_eval(model: nn.Module, loader: DataLoader, desc: str):
    ...
    video_labels_all = []
    video_mean_probs_all = []
    probs_with_all_all, probs_without_frames_all = [], []
    probs_without_memory_all, probs_without_frames_memory_all = [], []
    ...
    with torch.inference_mode(), \
        ...
        for frames, labels, lengths in tqdm(loader, desc=desc, leave=False):
            ...
            fl, fp, vl, vmp, p_all, p_no_frames, p_no_memory, p_no_both = _video_probs_from_forward(
                logits_with_all,
                logits_without_frames,
                logits_without_memory,
                logits_without_frames_memory,
            ...
            probs_with_all_all.extend(p_all.cpu().numpy().tolist())
            probs_without_frames_all.extend(p_no_frames.cpu().numpy().tolist())
            probs_without_memory_all.extend(p_no_memory.cpu().numpy().tolist())
            probs_without_frames_memory_all.extend(p_no_both.cpu().numpy().tolist())
...
def build_memory_bank(
    ...
    with torch.inference_mode(), \
         torch.cuda.amp.autocast(dtype=torch.float16):
        for frames, labels, lengths in tqdm(real_loader, desc="  Building bank", leave=False):
...
if __name__ == "__main__":
    ...
    print(
        ...
        f"  fusion_classifier:        TRAINABLE  (4096 temporal + 2 frame logits + 4096 memory)\n"
    ...
    for epoch in range(epochs):
        ...
        train_video_labels = []
        train_video_mean_probs = []
        train_with_all_probs, train_without_frames_probs = [], []
        train_without_memory_probs, train_without_frames_memory_probs = [], []
        ...
        for batch_idx, (frames, labels, lengths) in enumerate(
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16):
                ...
                loss = stage2_loss(logits_with_all, video_feats_list, labels, lam)
            ...
            train_with_all_probs.extend(p_all.cpu().numpy().tolist())
            train_without_frames_probs.extend(p_no_frames.cpu().numpy().tolist())
            train_without_memory_probs.extend(p_no_memory.cpu().numpy().tolist())
            train_without_frames_memory_probs.extend(p_no_both.cpu().numpy().tolist())
        ...
        compute_metrics(train_video_labels, train_with_all_probs, "Train (C) with all ", epoch)
        compute_metrics(train_video_labels, train_without_frames_probs, "Train (D) no frames", epoch)
        compute_metrics(train_video_labels, train_without_memory_probs, "Train (E) no memory", epoch)
        compute_metrics(train_video_labels, train_without_frames_memory_probs, "Train (F) no both  ", epoch)

        (
            val_fl, val_fp, val_vl, val_vmp,
            val_all, val_no_frames, val_no_memory, val_no_both,
        ) = run_eval(
        ...
        val_all_auc = compute_metrics(val_vl, val_all, "Val   (C) with all ", epoch)
        val_no_frames_auc = compute_metrics(val_vl, val_no_frames, "Val   (D) no frames", epoch)
        val_no_memory_auc = compute_metrics(val_vl, val_no_memory, "Val   (E) no memory", epoch)
        val_no_both_auc = compute_metrics(val_vl, val_no_both, "Val   (F) no both  ", epoch)
        print(
            f"  [Val AUC summary] with all={val_all_auc:.4f}  "
            f"without frames={val_no_frames_auc:.4f}  "
            f"without memory={val_no_memory_auc:.4f}  "
            f"without frames and memory={val_no_both_auc:.4f}"
        )

        (
            cdf_fl, cdf_fp, cdf_vl, cdf_vmp,
            cdf_all, cdf_no_frames, cdf_no_memory, cdf_no_both,
        ) = run_eval(
        ...
        test_auc = compute_metrics(cdf_vl, cdf_all, "Test  (C) with all ", epoch)
        test_no_frames_auc = compute_metrics(cdf_vl, cdf_no_frames, "Test  (D) no frames", epoch)
        test_no_memory_auc = compute_metrics(cdf_vl, cdf_no_memory, "Test  (E) no memory", epoch)
        test_no_both_auc = compute_metrics(cdf_vl, cdf_no_both, "Test  (F) no both  ", epoch)
        print(
            f"  [Test AUC summary] with all={test_auc:.4f}  "
            f"without frames={test_no_frames_auc:.4f}  "
            f"without memory={test_no_memory_auc:.4f}  "
            f"without frames and memory={test_no_both_auc:.4f}"
        )
        ...
        if test_auc > best_test_auc:
            ...
            print(f"\n  New best Test with-all AUC={best_test_auc:.4f} -> saved best.pth")
        else:
            print(f"\n  Best so far: epoch {best_epoch+1}  Test with-all AUC={best_test_auc:.4f}")

    print(f"\n{SEP}")
    print(f"  Stage 2 complete.")
    print(f"  Best checkpoint: epoch {best_epoch+1}  Test with-all AUC={best_test_auc:.4f}")
