...
* cls_list entries are (B*T, 1024) f_cls tensors (already squeezed by
  SpatialHead), not raw CLS tokens — VideoViT.forward reshapes them
  correctly, so train/eval code here is unchanged apart from index updates.
* frame_model_4layers.ViT.forward returns a 3-tuple
  (logits_list, features_list, cls_list) — there is no fused_list, so
  build_memory_bank() unpacks 3 values, not 4.
* Deepest SpatialHead is now index 3 (4 heads, 0-based) — already reflected
...
Outputs
-------
checkpoints_s2_4layers/latest.pth   — full VideoViT state dict after every epoch
checkpoints_s2_4layers/best.pth     — state dict of epoch with highest CDFv1 video AUC

Usage
-----
python train_stage2_4layers.py \
...
from video_model import RealVideoMemoryBank, TemporalTransformer, temporal_augment
...
parser = argparse.ArgumentParser(
    description="Stage 2: train temporal transformers with real+fake memory banks and frame logits appended"
)
...
parser.add_argument("--save_root",        default="checkpoints_s2_frame_end_dual_memory", type=str)
...
parser.add_argument("--cdfpp_root",       default="", type=str,
                    help="Optional CDF++/CDFv3 root for per-epoch video-level logging.")
parser.add_argument("--cdfpp_csv",        default="", type=str,
                    help="Optional CDF++/CDFv3 manifest CSV. If empty, common manifest names under --cdfpp_root are tried.")
parser.add_argument("--cdfpp_real_label", default=1, type=int,
                    help="Label value used for real samples in the CDF++ manifest. Remapped internally to real=0, fake=1.")
...
parser.add_argument("--use_memory_bank",  action="store_true",
                    help="Enable frozen real+fake kNN memory banks gated before temporal transformers.")
parser.add_argument("--knn_k",            default=32,   type=int,
                    help="Number of nearest neighbours to retrieve from each class bank.")
parser.add_argument("--memory_gate_init", default=0.13, type=float,
                    help="Initial sigmoid value for both learned memory gates.")
...
class VideoViT(nn.Module):
    ...
    def __init__(
        ...
        self.real_memory_bank: Optional[RealVideoMemoryBank] = None
        self.fake_memory_bank: Optional[RealVideoMemoryBank] = None
    ...
    def attach_memory_banks(self, real_bank: RealVideoMemoryBank, fake_bank: RealVideoMemoryBank):
        assert self.use_memory_bank, (
            "VideoViT was not constructed with use_memory_bank=True."
        )
        assert real_bank.num_heads == self.NUM_TEMPORAL_HEADS, (
            f"Real bank has {real_bank.num_heads} heads but model expects {self.NUM_TEMPORAL_HEADS}."
        )
        assert fake_bank.num_heads == self.NUM_TEMPORAL_HEADS, (
            f"Fake bank has {fake_bank.num_heads} heads but model expects {self.NUM_TEMPORAL_HEADS}."
        )
        self.real_memory_bank = real_bank
        self.fake_memory_bank = fake_bank
    ...
    def forward(
        ...
        if self.use_memory_bank:
            assert self.real_memory_bank is not None and self.fake_memory_bank is not None, (
                "use_memory_bank=True but banks are not attached. "
                "Call attach_memory_banks() first."
            )
            real_memory_refs = self.real_memory_bank.query(cls_sequences, key_padding_mask)
            fake_memory_refs = self.fake_memory_bank.query(cls_sequences, key_padding_mask)
        else:
            real_memory_refs = None
            fake_memory_refs = None

        video_feats_list = []
        no_memory_feats_list = [] if return_memory_ablation and real_memory_refs is not None else None
        for h, (temporal_tfm, frame_cls) in enumerate(zip(self.temporal_transformers, cls_sequences)):
            no_memory_frame_cls = frame_cls
            if real_memory_refs is not None:
                real_gate = torch.sigmoid(self.real_memory_gate[h]).to(dtype=frame_cls.dtype)
                fake_gate = torch.sigmoid(self.fake_memory_gate[h]).to(dtype=frame_cls.dtype)
                real_ref = real_memory_refs[h].unsqueeze(1)
                fake_ref = fake_memory_refs[h].unsqueeze(1)
                frame_cls = (
                    frame_cls
                    + real_gate * (real_ref - frame_cls)
                    + fake_gate * (fake_ref - frame_cls)
                )
    ...
    def load_image_weights(self, image_ckpt_path: str, strict: bool = False):
        ...
        state = ckpt.get("state_dict", ckpt.get("model_state_dict", ckpt.get("model", ckpt)))
...
class CDFv1VideoDataset(Dataset):
    """External test dataset (video-level). No augmentation at test time."""
    ...
    def __init__(
        ...
        print(f"{dataset_name} frames -> Real: {(df['label']==0).sum()} | "
        ...
        if skipped_vids:
            print(f"  [{dataset_name}] Skipped {skipped_vids} videos with no frames on disk")
        print(f"  [{dataset_name}] {len(self.videos)} videos loaded.")
...
def resolve_optional_manifest(root: str, csv_path: str, dataset_name: str) -> Optional[str]:
    if csv_path:
        return csv_path
    if not root:
        return None
    ...
    root_path = Path(root)
    candidates = [
        root_path / "manifest_cdfv3_face_crops.csv",
        root_path / "manifest_cdfpp.csv",
        root_path / "manifest_cdf++.csv",
        root_path / "manifest.csv",
    ]
    for candidate in candidates:
        if candidate.is_file():
            print(f"  [{dataset_name}] Using inferred manifest: {candidate}")
            return str(candidate)

    csv_candidates = sorted(root_path.glob("*.csv"))
    if len(csv_candidates) == 1:
        print(f"  [{dataset_name}] Using only CSV found under root: {csv_candidates[0]}")
        return str(csv_candidates[0])

    print(f"  [{dataset_name}] No manifest provided/found; skipping {dataset_name} evaluation.")
    return None


def eval_external_split(
    model: nn.Module,
    loader: DataLoader,
    dataset_name: str,
    epoch: int,
    ...
    fl, fp, vl, vmp, vwfp, vnfp, nm_wfp, nm_nfp = run_eval(
        model, loader, f"Epoch {epoch+1} [{dataset_name}]"
    )
    compute_metrics(fl, fp, f"{dataset_name:6s} (A) frame    ", epoch)
    compute_metrics(vl, vmp, f"{dataset_name:6s} (B) vid-mean ", epoch)
    if args.use_memory_bank:
        with_frame_auc = compute_metrics(vl, vwfp, f"{dataset_name:6s} (C) frame-end w/ mem ", epoch)
        no_frame_auc = compute_metrics(vl, vnfp, f"{dataset_name:6s} (D) no-frame  w/ mem ", epoch)
        no_mem_with_frame_auc = compute_metrics(vl, nm_wfp, f"{dataset_name:6s} (E) frame-end w/o mem", epoch)
        no_mem_no_frame_auc = compute_metrics(vl, nm_nfp, f"{dataset_name:6s} (F) no-frame  w/o mem", epoch)
        print(
            f"  [{dataset_name} AUC summary] frame-end w/ mem={with_frame_auc:.4f}  "
            f"frame-end w/o mem={no_mem_with_frame_auc:.4f}  "
            f"no-frame w/ mem={no_frame_auc:.4f}  "
            f"no-frame w/o mem={no_mem_no_frame_auc:.4f}"
        )
        write_grid_metric_rows(epoch, dataset_name.lower().replace("+", "p"), epoch_gate_avg, {
            "frame_end_w_mem": with_frame_auc,
            "frame_end_w_o_mem": no_mem_with_frame_auc,
            "no_frame_w_mem": no_frame_auc,
            "no_frame_w_o_mem": no_mem_no_frame_auc,
        })
        return with_frame_auc

    with_frame_auc = compute_metrics(vl, vwfp, f"{dataset_name:6s} (C) frame-end no mem", epoch)
    no_frame_auc = compute_metrics(vl, vnfp, f"{dataset_name:6s} (D) no-frame  no mem", epoch)
    print(
        f"  [{dataset_name} AUC summary] frame-end no mem={with_frame_auc:.4f}  "
        f"no-frame no mem={no_frame_auc:.4f}"
    )
    write_grid_metric_rows(epoch, dataset_name.lower().replace("+", "p"), epoch_gate_avg, {
        "frame_end_no_mem": with_frame_auc,
        "no_frame_no_mem": no_frame_auc,
    })
    return with_frame_auc
...
def build_memory_bank(
    ...
    """
    print(f"Building {bank_name} memory bank ...")

    class_indices = [i for i, (_, label) in enumerate(train_dataset.videos) if label == class_label]
    if not class_indices:
        raise ValueError(f"No videos with label={class_label} found for {bank_name} memory bank.")
    ...
    bank = RealVideoMemoryBank(
        ...
        num_heads = VideoViT.NUM_TEMPORAL_HEADS,  # 4 (was 5 with frame_42)
        k         = k,
    )
    ...
    with torch.inference_mode(), \
         torch.cuda.amp.autocast(dtype=torch.float16):
        for frames, labels, lengths in tqdm(class_loader, desc=f"  Building {bank_name} bank", leave=False):
            ...
            # No fused_list — this backbone's forward is a 3-tuple, not 4.
    ...
    print(f"  {bank_name} memory bank ready: {len(bank)} frozen CLS prototypes, k={k}")
    return bank
...
if __name__ == "__main__":
    ...
    cdf_dataset = CDFv1VideoDataset(
        ...
        dataset_name="CDFv1", real_label=0,
    )
    cdfpp_loader = None
    cdfpp_csv = resolve_optional_manifest(args.cdfpp_root, args.cdfpp_csv, "CDF++")
    if args.cdfpp_root and cdfpp_csv:
        cdfpp_dataset = CDFv1VideoDataset(
            cdfpp_csv, args.cdfpp_root, num_frames=NUM_FRAMES,
            dataset_name="CDF++", real_label=args.cdfpp_real_label,
        )
        cdfpp_loader = None  # created after loader settings are defined
    else:
        cdfpp_dataset = None
    ...
    print(f"CDFv1 per-epoch evaluation: ENABLED ({args.cdf_csv})")
    if cdfpp_dataset is not None:
        cdfpp_loader = DataLoader(
            cdfpp_dataset, batch_size=args.batch_size, shuffle=False,
            num_workers=_num_workers, pin_memory=True,
            collate_fn=video_collate_fn,
            persistent_workers=_persistent, prefetch_factor=_prefetch,
        )
        print("CDF++ per-epoch evaluation: ENABLED")
    else:
        print("CDF++ per-epoch evaluation: DISABLED")
    ...
    print(
        ...
        f"  memory_banks:             {'REAL+FAKE ENABLED' if args.use_memory_bank else 'DISABLED'}\n"
    ...
    if args.use_memory_bank:
        real_bank = build_memory_bank(
            model, train_dataset, k=args.knn_k, num_workers=_num_workers,
            class_label=0, bank_name="real-video",
        )
        fake_bank = build_memory_bank(
            model, train_dataset, k=args.knn_k, num_workers=_num_workers,
            class_label=1, bank_name="fake-video",
        )
        model.attach_memory_banks(real_bank, fake_bank)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (frames, labels, lengths) in enumerate(
            ...
            if args.use_memory_bank:
                ...
                if raw_gate_model.real_memory_gate is not None:
                    real_gate_avg = torch.sigmoid(raw_gate_model.real_memory_gate.detach()).mean().item()
                    fake_gate_avg = torch.sigmoid(raw_gate_model.fake_memory_gate.detach()).mean().item()
                    gate_avg = real_gate_avg + fake_gate_avg
                    batch_n = labels.numel()
                    gate_sum += gate_avg * batch_n
                    gate_real_sum += real_gate_avg * batch_n
                    gate_fake_sum += fake_gate_avg * batch_n
                    gate_count += batch_n
                    gate_real_count += batch_n
                    gate_fake_count += batch_n
        ...
        if args.use_memory_bank and gate_count:
            ...
            print(
                f"  Memory gates: real={gate_real_avg:.4f}  "
                f"fake={gate_fake_avg:.4f}  total_pull={gate_avg:.4f}"
            )
        ...
        compute_metrics(cdf_fl,  cdf_fp,  "CDFv1 (A) frame    ", epoch)
        compute_metrics(cdf_vl,  cdf_vmp, "CDFv1 (B) vid-mean ", epoch)
        if args.use_memory_bank:
            test_auc = compute_metrics(cdf_vl, cdf_vwfp,
                                       "CDFv1 (C) frame-end w/ mem ", epoch)
            test_no_frame_auc = compute_metrics(cdf_vl, cdf_vnfp,
                                                "CDFv1 (D) no-frame  w/ mem ", epoch)
            test_no_mem_with_frame_auc = compute_metrics(cdf_vl, cdf_nm_wfp,
                                                         "CDFv1 (E) frame-end w/o mem", epoch)
            test_no_mem_no_frame_auc = compute_metrics(cdf_vl, cdf_nm_nfp,
                                                       "CDFv1 (F) no-frame  w/o mem", epoch)
            print(
                f"  [CDFv1 AUC summary] frame-end w/ mem={test_auc:.4f}  "
            ...
            test_auc = compute_metrics(cdf_vl, cdf_vwfp,
                                       "CDFv1 (C) frame-end no mem", epoch)
            test_no_frame_auc = compute_metrics(cdf_vl, cdf_vnfp,
                                                "CDFv1 (D) no-frame  no mem", epoch)
            print(
                f"  [CDFv1 AUC summary] frame-end no mem={test_auc:.4f}  "
        ...
        if test_auc > best_test_auc:
            ...
            print(f"\n  ★ New best CDFv1 video AUC={best_test_auc:.4f} → saved best.pth")
        else:
            print(f"\n  Best so far: epoch {best_epoch+1}  CDFv1 frame-end AUC={best_test_auc:.4f}")

    print(f"\n{SEP}")
    print(f"  Stage 2 complete.")
    print(f"  Best checkpoint: epoch {best_epoch+1}  CDFv1 frame-end AUC={best_test_auc:.4f}")
