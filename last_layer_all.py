"""
Last-layer CLS + register + patch-attention Stage 1 training script.

This trains a DINOv3 ViT-L/16 model using only the final transformer layer:

  cls_feat   = final_layer_cls_token
  reg_feat   = mean_pool(final_layer_reg_tokens)
  patch_feat = AttentionPool(final_layer_patch_tokens)
  fused      = concat([cls_feat, reg_feat, patch_feat])
  logits     = classifier(fused)
  loss       = CrossEntropy(logits, labels) + lambda * SupCon(fused, labels)

There are no auxiliary tapped layers and no MultiSimilarity loss.
"""
...
parser = argparse.ArgumentParser(
    description="Stage 1 final-layer CLS+REG+PATCH-attn training with CE/SupCon"
)
parser.add_argument("--epochs",        default=50,   type=int)
parser.add_argument("--batch_size",    default=128,  type=int)
parser.add_argument("--num_workers",   default=36,   type=int)
parser.add_argument("--save_root",     default="checkpoints_last_layer_all", type=str)
...
class LastLayerAllViT(nn.Module):
    """DINOv3 ViT-L/16 using final-layer CLS, REG, and attention-pooled patches."""
...
def all_loss(logits, fused_features, labels, supcon_weight):
    return ce_loss(logits, labels) + supcon_weight * supcon_loss(fused_features, labels)
...
if __name__ == "__main__":
    ...
    model = LastLayerAllViT().to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imgs, labels) in enumerate(
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16):
                logits, fused_features = model(imgs)
                loss = all_loss(logits, fused_features, labels, args.supcon_weight)
