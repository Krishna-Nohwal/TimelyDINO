"""
Visualize DINOv3 embeddings across a scale pyramid on CDFv1 images.
...
Example
-------
python visualize_dinov3_scale_pyramid_cdfv1.py ^
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="DINOv3 scale-pyramid similarity/UMAP visualization on CDFv1."
    )
    parser.add_argument("--cdf_root", default=r"E:\Work\cdfv1_onct_out")
    parser.add_argument(
        "--cdf_csv",
        default=r"E:\Work\cdfv1_onct_out\manifest_cdfv1_onct.csv",
    )
...
def load_cdfv1_items(cdf_root: str, cdf_csv: str) -> list[dict]:
    ...
    for _, row in df.iterrows():
        ...
        if path.is_file():
            items.append({
                "image_path": str(path),
                "sample_dir": rel,
                "video_id": video_id_from_sample_dir(rel),
                "label": int(row["label"]),
            })
        else:
            skipped += 1
    print(f"Loaded CDFv1 manifest rows: found={len(items)} skipped_missing={skipped}")
    return items
...
def plot_average_heatmap(scale_pair_values: np.ndarray, scales, out_path: Path, distance: str):
    ...
    measure_name, measure_word, _ = metric_display(distance)
...
def main():
    ...
    print("=" * 88)
    print("DINOv3 scale-pyramid visualization on CDFv1")
    print("=" * 88)
    ...
    items = load_cdfv1_items(args.cdf_root, args.cdf_csv)
