"""
Visualize DINOv3 embeddings across a scale pyramid on FF++ images.

For each sampled FF++ frame image:
...
Example
-------
python visualize_dinov3_scale_pyramid_ffpp.py \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="DINOv3 scale-pyramid similarity/UMAP visualization on FF++."
    )
    parser.add_argument(
        "--manifest",
...
def load_ffpp_items(manifest: str, root_dir: str) -> list[dict]:
    ...
    for _, row in df.iterrows():
        ...
        if path.is_file():
            ...
            skipped += 1
    print(f"Loaded FF++ manifest rows: found={len(items)} skipped_missing={skipped}")
    return items
...
def main():
    ...
    print("=" * 88)
    print("DINOv3 scale-pyramid visualization on FF++")
    print("=" * 88)
    ...
    items = load_ffpp_items(args.manifest, args.root_dir)
