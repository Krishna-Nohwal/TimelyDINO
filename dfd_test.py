"""
dfd_knn42_test.py  —  Video-level AUC evaluation on DFD (manipulated_sequences
                       / original_sequences) using a Stage 2 VideoViT
                       checkpoint  (video_model_knn.VideoViT).

Adapted from the df1.0 evaluation script to the DFD dataset. Both datasets
are pushed through the same extract_frames.py + preprocess_dfd.py pipeline,
so they end up in the IDENTICAL nested per-sample-subdirectory layout, NOT
flat "<video>_<frame>.png" files:

    <fake_root>/<video_id>/<frame_idx>/image.png
    <real_root>/<video_id>/<frame_idx>/image.png

    e.g. .../dfd_faces/fake/01_02__meeting_serious__8aa367dd/0/image.png
         .../dfd_faces/real/01__meeting_serious__80bc9823/31/image.png
...
extracted by extract_frames.py + preprocess_dfd.py. Padding here is a
defensive fallback (repeating the last available frame) in case any videos
have missing/failed frames -- it should rarely trigger since preprocessing
already fixes every video at 32 frames.
...
Usage
-----
python dfd_knn42_test.py \
    --checkpoint  checkpoints_s2/best.pth \
    --fake_root   /media/tarun/B482367C823642E2/usr/dfd_faces/fake \
    --real_root   /media/tarun/B482367C823642E2/usr/dfd_faces/real \
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Evaluate a Stage 2 VideoViT checkpoint on DFD."
    )
    ...
    p.add_argument("--fake_root",
                   default="/media/tarun/B482367C823642E2/usr/dfd_faces/fake",
                   help="Root dir containing fake video subdirectories "
                        "(<fake_root>/<video_id>/<frame_idx>/image.png).")
    p.add_argument("--real_root",
                   default="/media/tarun/B482367C823642E2/usr/dfd_faces/real",
    ...
    p.add_argument("--train_real_root", default="/media/tarun/B482367C823642E2/usr/ff++/onct_preprocessed_out/real", type=str,
                   help="Root dir of real training frames (required when checkpoint "
                        ...
                        "<train_real_root>/<sample_dir>/image.png")
    p.add_argument("--knn_k",           default=8,  type=int,
...
# DFD frames are pre-extracted/padded to a fixed 32 frames per video by
...
# ---------------------------------------------------------------------------
# Video ID helper  (DFD nested per-sample-subdirectory layout)
# ---------------------------------------------------------------------------
#
# DFD (as produced by preprocess_dfd.py) is laid out as:
...
class DfdFrameDataset(Dataset):
    """
    Loads individual frames from DFD's nested per-video-subdirectory
    ...
    def __init__(self, fake_root: Path, real_root: Path):
        ...
        print(f"  DFD frames    ->  Real: {len(real_samples)}  |  "
...
class DfdClipDataset(Dataset):
    ...
    def __init__(self, fake_root: Path, real_root: Path, num_frames: int):
        ...
        print(f"  DFD clips     ->  Real: {real_n}  |  Fake: {fake_n}  |  "
...
class TrainRealClipDataset(Dataset):
    ...
    --real_root/--fake_root (DFD's <video_id>/<frame_idx>/image.png
...
def main():
    ...
    sep = "\u2550" * 72
    print(f"\n  {sep}")
    print(f"  DFD EVALUATION  --  VideoViT (Stage 2)")
    print(f"  {sep}")
    ...
    # -- Datasets & loaders --------------------------------------------------
    frame_dataset = DfdFrameDataset(fake_root, real_root)
    clip_dataset  = DfdClipDataset(fake_root, real_root, num_frames=args.num_frames)
