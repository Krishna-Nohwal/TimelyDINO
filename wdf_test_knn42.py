"""
wdf_knn42_test.py  —  Video-level AUC evaluation on WDF (wild-deepfake style
                       dataset: /media/.../wdf/test/{real,fake}) using a
                       Stage 2 VideoViT checkpoint  (video_model_knn.VideoViT).

Adapted from the DFD/df1.0 evaluation script to the WDF dataset, which is
laid out as FLAT FILES with NO per-video subdirectories and no separate raw
video / extraction step -- frames are already extracted and dumped directly
into --fake_root / --real_root:

    <fake_root>/<video_id>_<frame_number>.png
    <real_root>/<video_id>_<frame_number>.png

    e.g. .../wdf/test/real/0_102.png    -> video_id "0",   frame 102
         .../wdf/test/real/0_1006.png   -> video_id "0",   frame 1006
         .../wdf/test/fake/258_60.png   -> video_id "258", frame 60

video_id is simply the leading integer before the first underscore; frame
numbers are NOT sequential/zero-based and vary per video (observed range:
roughly 25-90+ frames per video, non-contiguous numbering) -- this is
expected and handled by uniformly sub/super-sampling each video's available
frames down to --num_frames via the same _sample_frame_indices() logic
already used for DFD/df1.0 (no filename-based padding assumptions needed).

This is the same flat "<video>_<frame>.png" layout the very first DFDC
version of this script targeted, just with a simpler video_id (no embedded
underscores) -- so grouping is a plain "split on last underscore" instead of
a full DFDC-style regex walk.
...
Directory layout
----------------
  <fake_root>/<video_id>_<frame_number>.png   (flat, no per-video subdirs)
  <real_root>/<video_id>_<frame_number>.png

  video_id is everything before the LAST underscore; frame_number is the
  trailing integer. All frames sharing a video_id are grouped into one
  video, sorted by frame_number, then uniformly sampled to --num_frames.
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Evaluate a Stage 2 VideoViT checkpoint on WDF."
    )
    ...
    p.add_argument("--fake_root",
                   ...
                   help="Root dir containing flat fake frame images "
                        "(<fake_root>/<video_id>_<frame_number>.png, no subdirs).")
    p.add_argument("--real_root",
                   default="/media/tarun/B482367C823642E2/usr/wdf/test/real",
                   help="Root dir containing flat real frame images "
                        "(<real_root>/<video_id>_<frame_number>.png, no subdirs).")
...
# WDF ships pre-extracted frames with widely varying per-video frame counts
# (observed range ~25-90+ frames/video). TARGET_FRAME_COUNT here is only
# used as the padding floor for short videos in group_and_pad_videos();
# --num_frames (default 32, passed separately to Df1ClipDataset/WdfClipDataset)
# is what actually determines how many frames the temporal transformer sees
# per video -- _sample_frame_indices() uniformly downsamples videos with more
# than num_frames and tiles videos with fewer, so long WDF videos are handled
# correctly without any truncation happening here.
...
# ---------------------------------------------------------------------------
# Video ID helper  (WDF flat-file layout, no per-video subdirectories)
# ---------------------------------------------------------------------------
#
# WDF frames are pre-extracted and dumped flat as:
#     <root>/<video_id>_<frame_number>.png
# e.g. "0_102.png" -> video_id "0", frame_number 102
#      "258_60.png" -> video_id "258", frame_number 60
#
# Unlike DFDC's original convention (prefix may itself contain underscores,
# e.g. "0_4003_131.png" -> video "0_4003"), WDF video IDs observed so far are
# plain integers with no embedded underscores, so grouping is a simple
# "split on the LAST underscore" rather than a full prefix-matching regex.
# The regex below still tolerates an underscore-containing video_id prefix
# (matching greedily up to the last "_<digits>.png"), so it degrades safely
...
def group_and_pad_videos(root: Path, target_frame_count: int):
    """
    Walk flat <root>/<video_id>_<frame_number>.png files and group into
    per-video lists of (frame_number, path), sorted numerically by
    frame_number.

    Videos with fewer than target_frame_count available frames are padded
    by repeating their last frame. Videos with MORE than target_frame_count
    are left as-is (not truncated here) -- downstream _sample_frame_indices()
    uniformly sub-samples them to whatever --num_frames is requested.

    Returns: dict video_id -> [(frame_number, Path), ...] (length is
             max(n_available, target_frame_count); padded entries repeat the
             last (frame_number, path) tuple).
    """
    ...
    for f in sorted(root.iterdir()):
        if not f.is_file():
            continue
        m = FRAME_FILE_RE.match(f.name)
        if not m:
            skipped += 1
            continue
        vid = m.group(1)
        frame_num = int(m.group(2))
        vid2items[vid].append((frame_num, f))

    if skipped > 0:
        print(f"    [{root.name}] {skipped} file(s) did not match "
              f"'<video_id>_<frame_number>.png' and were skipped")
...
def collect_samples(root: Path, label: int, target_frame_count: int):
    """Flat file layout; short videos padded to target_frame_count."""
...
class WdfFrameDataset(Dataset):
    """
    Loads individual frames from WDF's flat <video_id>_<frame_number>.png
    ...
    def __init__(self, fake_root: Path, real_root: Path):
        ...
        print(f"  WDF frames    ->  Real: {len(real_samples)}  |  "
...
class WdfClipDataset(Dataset):
    ...
    def __init__(self, fake_root: Path, real_root: Path, num_frames: int):
        ...
        print(f"  WDF clips     ->  Real: {real_n}  |  Fake: {fake_n}  |  "
...
class TrainRealClipDataset(Dataset):
    ...
    --real_root/--fake_root (WDF's flat <video_id>_<frame_number>.png
    files, with no per-video subdirectories at all). Each FF++ sample is its own
...
def main():
    ...
    sep = "\u2550" * 72
    print(f"\n  {sep}")
    print(f"  WDF EVALUATION  --  VideoViT (Stage 2)")
    print(f"  {sep}")
    ...
    clip_dataset  = WdfClipDataset(fake_root, real_root, num_frames=args.num_frames)
