...
Usage
-----
python dfdc_knn42.py \
    --checkpoint  checkpoints_s2/best.pth \
    --fake_root   /media/tarun/B482367C823642E2/usr/dfdc/fake \
    --real_root   /media/tarun/B482367C823642E2/usr/dfdc/real \
    [--num_frames 30] [--batch_size 4] [--num_workers 4] \
    [--topk 10] [--no_compile] [--fp32] \
    [--real_bias 0.0] [--save_results results.csv]

Directory layout
----------------
  <fake_root>/<image>.png   (flat, no per-sample subdirectories)
  <real_root>/<image>.png

  Filenames encode video + frame as a trailing "_<frame_number>" token,
  where the prefix itself may contain underscores:
      0_4003_131.png     -> video id "0_4003"
      0_6111_1.png       -> video id "0_6111"

  Frame numbers increment by 10 (1, 11, 21, ... up to 291). Many DFDC
  videos have fewer than 30 extracted frames (dropped face detections,
  short clips, etc.) -- ALL videos are used; those with fewer than 30
  frames are padded by repeating their last frame (same policy as DFD).
"""
...
def parse_args():
    ...
    p.add_argument("--fake_root",
                   default="/media/tarun/B482367C823642E2/usr/dfdc/fake",
                   help="Root dir containing fake frame images (flat directory).")
    p.add_argument("--real_root",
                   default="/media/tarun/B482367C823642E2/usr/dfdc/real",
                   help="Root dir containing real frame images (flat directory).")
    ...
    p.add_argument("--train_real_root", default="/media/tarun/B482367C823642E2/usr/ff++/onct_preprocessed_out/real", type=str,
                   help="Root dir of real training frames (required when checkpoint "
                        ...
                        "Same layout as CDFv2's original per-sample-subdirectory format: "
...
# DFDC frame extraction is uneven; videos with fewer than TARGET_FRAME_COUNT
# frames are padded by repeating their last frame (same policy as DFD).
...
# Matches "<prefix>_<frame_number>.<ext>" where prefix may itself contain
# underscores, e.g. "0_01__exit_phone_room_131.png" -> prefix "0_01__exit_phone_room".
FRAME_FILE_RE = re.compile(r"^(.*)_(\d+)\.(png|jpg|jpeg)$", re.IGNORECASE)


def video_id_from_sample(filename: str) -> str:
    """
    Strip trailing "_<frame_number>.<ext>" to get the video ID.

      '0_01__exit_phone_room_131.png'  -> '0_01__exit_phone_room'
      '0_01__hugging_happy_1.png'      -> '0_01__hugging_happy'
    """
    m = FRAME_FILE_RE.match(filename)
    if not m:
        raise ValueError(f"Filename does not match expected pattern: {filename}")
    return m.group(1)


def group_and_pad_videos(root: Path):
    """
    Group flat-directory frame files by video prefix, sorted numerically by
    frame number. Videos with fewer than TARGET_FRAME_COUNT frames are
    padded by repeating their last frame; videos with more are left as-is
    (should not normally happen given the dataset's frame-number range).

    Returns: dict video_id -> [(frame_num, Path), ...] of length
             TARGET_FRAME_COUNT (padded entries repeat the last (num, path)
             tuple, so frame_num is not unique for padded videos).
    """
...
def collect_samples(root: Path, label: int):
    """Flat directory of images; short videos padded to TARGET_FRAME_COUNT."""
    grouped = group_and_pad_videos(root)
    samples = []
    for vid, items in grouped.items():
        for frame_num, f in items:
...
class CDFv2FrameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img      = normalize(img)
        video_id = video_id_from_sample(f.name)
...
class TrainRealClipDataset(Dataset):
    ...
    NOTE: this dataset points at the FF++ training data, which uses a
    DIFFERENT directory layout than --real_root/--fake_root (DFDC flat
    files). Each sample is its own subdirectory containing "image.png",
    with video ID encoded via a trailing "_fNNNN" or "_frame_NN" suffix
    on the subdirectory name. Do not confuse this with FRAME_FILE_RE
    (used for the DFDC flat-file layout above) -- they are unrelated.
...
def main():
    ...
    auc_frame_end = compute_and_print_metrics(
        ...
        f"(C) Video-temporal frame-end{bias_tag}  (CDFv2)"
    )
    auc_no_frame = compute_and_print_metrics(
        vid_labels_clip, vid_no_frame_probs,
        f"(D) Video-temporal no-frame{bias_tag}  (CDFv2)"
    )
